...
class OpenAICompatClient:
        ...
        *,
        ...
        req = request.Request(
            ...
            headers={
                ...
                'User-Agent': 'claw-code-agent/0.1',
            },
            method='POST',
        )
        response = self._open_with_retries(req, request_kind='Stream')
        try:
            with response:
            ...
            raise OpenAICompatError(
                f'Stream from local model backend timed out after {self.config.timeout_seconds:g}s'
            ) from exc
        except OSError as exc:
            raise OpenAICompatError(
                f'Stream from local model backend failed: {exc}'
            ) from exc
    ...
    def _request_json(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        raw = self._urlopen_with_retries(req, request_kind='Request')
    ...
    def _open_with_retries(self, req: request.Request, *, request_kind: str):  # noqa: ANN202
        attempts = 3
        for attempt in range(attempts):
            try:
                ...
                if exc.code in self._TRANSIENT_HTTP_CODES and attempt < attempts - 1:
                ...
                if attempt < attempts - 1:
                    ...
                    continue
                raise OpenAICompatError(
                    f'{request_kind} to local model backend timed out after {self.config.timeout_seconds:g}s'
                ) from exc
            except OSError as exc:
                if attempt < attempts - 1:
                    time.sleep(0.5 * (attempt + 1))
                    continue
                raise OpenAICompatError(
                    f'{request_kind} to local model backend failed: {exc}'
                ) from exc
        raise OpenAICompatError(f'{request_kind} to local model backend failed after retries')

    def _urlopen_with_retries(self, req: request.Request, *, request_kind: str) -> bytes:
        attempts = 3
        for attempt in range(attempts):
            try:
