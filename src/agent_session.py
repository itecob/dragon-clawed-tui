...
class AgentSessionState:
    ...
    def to_openai_messages(self) -> list[JSONDict]:
        return _replayable_openai_messages(self.messages)
    ...
    def from_persisted(
        ...
        *,
        ...
        loaded_messages = [AgentMessage.from_openai_message(message) for message in messages]
        normalized_messages: list[AgentMessage] = []
        seen_initial_system = False
        for message in loaded_messages:
            if message.role == 'system':
                if not normalized_messages and not seen_initial_system:
                    seen_initial_system = True
                    normalized_messages.append(message)
                    continue
                metadata = dict(message.metadata)
                metadata.setdefault('original_role', 'system')
                metadata['message_role'] = 'user'
                normalized_messages.append(replace(message, role='user', metadata=metadata))
                continue
            normalized_messages.append(message)
        return cls(
            system_prompt_parts=tuple(system_prompt_parts),
...
def _replayable_openai_messages(messages: list[AgentMessage]) -> list[JSONDict]:
    replayable: list[JSONDict] = []
    pending_tool_ids: set[str] = set()
    buffered_tool_turn: list[JSONDict] = []

    def flush_tool_turn_if_complete() -> None:
        nonlocal buffered_tool_turn
        if buffered_tool_turn and not pending_tool_ids:
            replayable.extend(buffered_tool_turn)
            buffered_tool_turn = []

    def drop_incomplete_tool_turn() -> None:
        nonlocal buffered_tool_turn, pending_tool_ids
        buffered_tool_turn = []
    ...
    for message in messages:
        role = message.role
        if role == 'assistant':
            flush_tool_turn_if_complete()
            if buffered_tool_turn:
                drop_incomplete_tool_turn()
            ...
            if message.tool_calls:
                tool_call_ids = {
                    ...
                    if isinstance(tool_call.get('id'), str) and str(tool_call.get('id')).strip()
                }
                ...
                buffered_tool_turn = [payload]
                continue
            replayable.append(payload)
            continue
        if role == 'tool':
            tool_call_id = message.tool_call_id
            if buffered_tool_turn and tool_call_id in pending_tool_ids:
                buffered_tool_turn.append(message.to_openai_message())
                pending_tool_ids.remove(tool_call_id or '')
                flush_tool_turn_if_complete()
            continue
        flush_tool_turn_if_complete()
        if buffered_tool_turn:
            drop_incomplete_tool_turn()
        replayable.append(message.to_openai_message())

    flush_tool_turn_if_complete()
    return _drop_extra_trailing_assistants(replayable)


def _drop_extra_trailing_assistants(messages: list[JSONDict]) -> list[JSONDict]:
    trailing_assistants = 0
    for message in reversed(messages):
        if message.get('role') != 'assistant':
            break
        trailing_assistants += 1
    if trailing_assistants <= 1:
        return messages
    return messages[: -(trailing_assistants)] + messages[-1:]
